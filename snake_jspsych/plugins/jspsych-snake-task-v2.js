...
var jsPsychSnakeTask = (function (jspsych) {
  ...
  class Trial {
    ...
    checkCollisions() {
      ...
      // 检查上一帧吃苹果的豁免位置是否仍在1秒有效期内
      const lastEatenPos = this._eatenFoodPos;
      if (lastEatenPos && performance.now() - lastEatenPos.time > 1000) {
        // 超过1秒，清除豁免
        this._eatenFoodPos = null;
      }
      ...
      if (foodIdx !== -1) {
        ...
        // 标记该草丛1秒内豁免（防止3×3拾取后蛇头经过时误冻结）
        this._eatenFoodPos = { x: food.x, y: food.y, time: performance.now() };
